...
class GUI():
    ...
    def __init__(self,root,params):
        ...
        self.button_start_stop_sim = Button(root,text="Start",command=self.start_stop_sim,width=15)
        ...
        self.button_start_stop_sim.grid(column=0,row=3,pady=5,sticky='nw')
        self.button_quit.grid(column=0,row=5,pady=5,sticky='nw')

    #button functions
    def start_stop_sim(self):
        if self.stop_param:
            self.stop_param = False
            self.button_start_stop_sim.config(text="Stop")
        else:
            self.stop_param = True
            self.button_start_stop_sim.config(text="Start")
